...
const Statistics = () => {
  const transactions = useAppSelector(selectTransactions) as Transaction[];
  ...
  const grouped = useMemo(() => {
    return expenses.reduce<Record<string, number>>((acc, t) => {
      const category = t.category ? t.category.trim() : 'Uncategorized';
      const sum = typeof t.sum === 'string' ? parseFloat(t.sum) || 0 : t.sum;
  ...
  const centerTextPlugin: Plugin<'doughnut'> = {
    ...
    beforeDraw: (chart) => {
      ...
      const total = dataset
        ? (dataset.data as number[]).reduce((acc, v) => acc + v, 0)
        : 0;
